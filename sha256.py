...
def sha_256(input_string):
    ...
    final_hash = (bin_return(hash_0) + bin_return(hash_1) + bin_return(hash_2) + bin_return(hash_3) + bin_return(hash_4) + bin_return(hash_5) + bin_return(hash_6) + bin_return(hash_7))
    return(final_hash)
inp = input("String: ")
while 1:
    sha = sha_256(inp)
    if sha[1:11] == "0000000000":
        print("Hash:\n"+sha)
    inp = sha
